...
export function makeRpcHandler(kernel: MultimanKernel): RpcHandler {
  const routes: Record<string, (p: Params) => unknown | Promise<unknown>> = {
    ...
    "task.transition": (p) => kernel.transition(
      reqStr(p, "id"), reqStr(p, "to") as TaskStatus, optStr(p, "reason") ?? "rpc",
      { roleId: optStr(p, "roleId") },
    ),
    "task.claim": (p) => kernel.claimNextTask(reqStr(p, "roleId")),
    "task.report": (p) => kernel.reportTask(reqStr(p, "id"), {
      status: reqStr(p, "status") as TaskStatus, result: optStr(p, "result"),
      error: optStr(p, "error"), sessionId: optStr(p, "sessionId"),
    }),
    "task.heartbeat": (p) => { kernel.heartbeat(reqStr(p, "id"), reqStr(p, "roleId")); return { ok: true } },
    // role.* (S0 scope)
    "role.create": (p) => kernel.createRoleViaDao(
      reqStr(p, "name"), reqStr(p, "kind") as RoleKind,
      optStr(p, "instructions"), optStr(p, "vendor"), optStr(p, "model"),
    ),
