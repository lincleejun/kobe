...
| ---------------- | --------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ctrl+1..9`      | `focus.numeric` (global, 1–4) vs `chat.tab.pick` (workspace, 1–9) | Chat tab pick **only registers when there's >1 chat tab open**. Single-tab is the common case → ctrl+1..4 always means pane focus. Multi-tab in workspace shadows pane-focus on the digits 1..9. Trade documented in `Chat.tsx` (search "muscle memory"). |
