...
| ---------------- | --------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ctrl+1..4`      | `focus.numeric` (global) — needs CSI-u / kitty keyboard | Most terminals (Terminal.app, iTerm default, tmux without extended-keys) drop the ctrl modifier on digit keys — pressing `ctrl+1` just sends the byte `1`, no modifier info. Kobe enables the kitty/CSI-u keyboard protocol via `useKittyKeyboard: {}` in `render()` (see `app.tsx`). Supporting terminals (kitty, foot, recent iTerm2, recent Terminal.app, Ghostty, recent WezTerm) reply to the enable sequence and start sending CSI-u events. **tmux users** must add `set -g extended-keys on` (tmux ≥ 3.2) to `~/.tmux.conf` for the sequences to pass through. |
| `ctrl+q` (any pane) | always-works fallback for "back to tasks" | Promoted to global because ctrl+letter chords map to stable C0 control bytes that ALL terminals propagate — no CSI-u / extended-keys dance. So users on plain Terminal.app + default tmux can still escape to the sidebar with `ctrl+q` even when `ctrl+1` is silently dropped. |
