...
| ---------------- | --------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ctrl+1..4`      | `focus.numeric` (global) wins everywhere | **Pane focus has hard precedence.** Chat tab pick was originally `ctrl+1..9` and shadowed pane focus in workspace + multi-tab. Moved to `alt+1..9` (id `chat.tab.pick`, scope workspace) so pane-focus chords are uniform across panes — every pane reacts the same way to `ctrl+N`. |
